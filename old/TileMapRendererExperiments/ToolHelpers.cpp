...
glm::vec2 EditorToolHelpers::GetSnappedPosition(const glm::vec2& worldpsacePosition, f32 tileDivider)
{
    ...
    return glm::vec2(
        RoundToMultiple(worldpsacePosition.x, 1.0f / (f32)tileDivider) - 0.5f,
        RoundToMultiple(worldpsacePosition.y, 1.0f / (f32)tileDivider) - 0.5f
    );
}

f32 EditorToolHelpers::RoundToMultiple(f32 val, f32 step)
{
